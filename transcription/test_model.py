...
for question, responses in good_responses.items():
    data.extend(responses)
    labels.extend([1] * len(responses))  # Label 1 for good responses

for question, responses in bad_responses.items():
    data.extend(responses)
    labels.extend([0] * len(responses))  # Label 0 for bad responses

# Split the data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(data, labels, test_size=0.2, random_state=42)

# Build the pipeline with a vectorizer and a classifier
model = make_pipeline(CountVectorizer(stop_words=stopwords.words('french')), MultinomialNB())
